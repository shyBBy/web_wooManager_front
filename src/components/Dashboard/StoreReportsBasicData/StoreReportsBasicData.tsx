...
export const StoreReportsBasicData = () => {
    const { user } = useAuth();
    const [salesReport, setSalesReport] = useState<GetOneSalesReportResponse | null>(null);
    ...
    return (
        <Grid container spacing={3}>
            <Grid item xs={12} sm={6} md={6} lg={3}>
                <MainCard
                    title={"Klienci"}
                    count={loading ? <CircularProgress /> : salesReport?.total_customers}
            ...
            <Grid item xs={12} sm={6} md={6} lg={3}>
                <MainCard
                    title={"Zamówienia"}
                    count={loading ? <CircularProgress /> : salesReport?.total_orders}
            ...
            <Grid item xs={12} sm={6} md={6} lg={3}>
                <MainCard
                    title={"Produkty"}
                    count={loading ? <CircularProgress /> : salesReport?.total_items}
            ...
            <Grid item xs={12} sm={6} md={6} lg={3}>
                <MainCard
                    ...
                    count={loading ? <CircularProgress /> : salesReport?.total_sales}
